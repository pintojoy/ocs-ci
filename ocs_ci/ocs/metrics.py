    ...
    for minfo in metrics:
        ...
        try:
            if parms["pod"] and parms["device"]:
                if parms["pod"] not in ret_dict:
                    ret_dict[parms["pod"]] = {}
                if parms["device"] not in ret_dict[parms["pod"]]:
                    ret_dict[parms["pod"]][parms["device"]] = {}
                entry = ret_dict[parms["pod"]][parms["device"]]
                if minfo["stat_name"] not in entry:
                    entry[minfo["stat_name"]] = []
                ent_info = {}
                ent_info["numbers"] = minfo["numbers"]
                if "operation" in parms:
                    ent_info["operation"] = parms["operation"]
                entry[minfo["stat_name"]].append(ent_info)
        except KeyError:
